$(function() {	
	$('li.tab-title a').each(function() { //workaround for foundation tabs
...
function initCategoriesUpdate(options) {
	...
	// Verwijder event listeners van niet bestaande elementen
	var updateValidationListeners = function() {
		var yiiFormSettings = $container.closest('form').data('settings');
		yiiFormSettings.attributes = $.grep(yiiFormSettings.attributes, function(attribute) {
			return document.getElementById(attribute.inputID) !== null;			
		});
	};
...
function initCurrencyFields(options) {
	var pref = document.getElementById( options.model + '_preferred_currency' );
	
	var updatePrice = function() {
		var currency = $(pref).val();
		
		var isCurrency = function() {
			return $(this).data('currency') === currency;
		};
		var isNotCurrency = function() {
			return $(this).data('currency') !== currency;
		};
		
		var price = $(".price");
		
		price.filter(isCurrency).addClass('current');
		price.filter(isNotCurrency).removeClass('current');
	};
	
	$(pref).change(updatePrice);
	updatePrice();
}
...
function initMaxAnswers(options) {
	
	var addButton = $("#addOption");
	var container = $("#optioncontainer");
	var maxAnswers = $(options.maxAnswers); // De selector van de select met max answers
	var emptyOption = $(options.maxAnswers+' :first-child').html();
	
	var updateMaxAnswers = function() {
		amountOfAnswers = container.children('div[data-group]').length;
		selected = maxAnswers.val();
		
		options = '<option value="">'+emptyOption+'</option>';
		for(i = 1; i < amountOfAnswers; i++){
			options += '<option value="'+i+'">'+i+'</option>';
		}
		
		maxAnswers.html(options);
		maxAnswers.val(selected);
	};
...
function initStatsPage(okTxt, cancelTxt) {
	var $dialog = $("#session-confirm-delete");
	var $link = $("#session-action-delete");
	
	var buttons = {};
	buttons[okTxt] = function() {
		$form = $('#SessionSelectionForm');
		$form.attr('action', $link.prop('href')).submit();
...
function initSettingsPlayerInfo(options) {
	...
	$('.custom-question__input').change(function(){
		var customSelectfield = $(this).closest('.custom-question').find('.custom-question__selectfield');
		if ( $(this).val() ) {
			customSelectfield.show();
		} else {
			customSelectfield.hide();
		}
	}).change();
};
...
function initDialCode(options){
	var watchField = options.watchField;
	var preview = options.preview;
	
	$(watchField).on('change', function(e){
		dialcode = $(this).find(':selected').data('dialcode');
		if(dialcode)
			$(preview).text(dialcode);
		else 
			$(preview).text(' ');
	});
	
	$(watchField).trigger('change');
}
...
function initChartPreview() {
	...
	google.load('visualization', '1', {
		'packages': ['corechart'],
		'callback': function() {
			var $element = $('[data-chart-preview]');
			var previewData = google.visualization.arrayToDataTable(eval($element.data('chart-preview')));
			var $previewContainer = $('<div data-preview-container />');
			
			
			showChart = function(chartType, options) {
				var $container = $('.' + chartType, $previewContainer);
				
				if (!$container.size()) {
					$container = $('<div class="'+chartType+' chart" />');
					$previewContainer.append($container);
				}
				$container.show();				
				var chart = eval('new google.visualization.' + chartType + '($container.get(0));');
				chart.draw(previewData, eval(options));				
			}
			
			$element.parents('div:first').after($previewContainer);
			$element.change(function() {
				$('.chart', $previewContainer).hide();
				var $option = $('[value="'+$element.val()+'"]', $element);
				if ($option.data('bar-horizontal')) {
					showChart('BarChart', $option.data('bar-horizontal'));
				}
				else if ($option.data('bar-vertical') ){
					showChart('ColumnChart', $option.data('bar-vertical'));				
				}
...
function beforeCopyQuestionValidate() {
	var $dialog = $('#modal-copied-question');
	if (!$dialog.data('done')) {
...
function initCopyQuestion() {
	var $dialog = $('#modal-copied-question');
	$('a', $dialog).click(function(e) {
		e.preventDefault();
		$dialog.data('done', true);
		$($(this).data('target')).prop('checked', true).parents('form').submit();
	});
}
